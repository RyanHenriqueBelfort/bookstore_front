...
export function Gender() {
  ...
  function handleDelete() {
    const allIdsGender = book.filter((data) => {
      return data.gender_id == selectBook;
    });
    if (allIdsGender.length > 0) {
      alert("Gênero já está em uso");
    } else {
          ...
          <Td>
            <Stack direction="row" spacing={5}>
              <ButtonTable
                ...
                onClick={() => router.push(`/edit/gender/${dados.id}`)}
              >
                Editar
              </ButtonTable>
              <ButtonTable
