...
export default function Index() {
  ...
  useEffect(() => {
    ...
    currentBook.then((value) => {
      if (value) {
        reset({
          autor: value.author_id,
          titulo: value.title,
          ano: value.release_year,
          genero: value.gender_id,
          editora: value.publisher_id,
        });
      }
    });
  }, [book]);
  ...
  const onSubmit = (data) => {
    api.put(`/book/${currentRouter}`, {
      ...
      .then(() => api.get("/book").then((response) => setBook(response.data)))
    ...
    >
      ...
      >
        <Stack spacing={5}>
          ...
          <SelectInput label="Autor" {...register("autor")}>
            {author.map((data) => {
              return (
                <option
                  style={{ color: "black" }}
                  key={data.id}
                  selected={values.author_id == data.id}
                  value={data.id}
                >
                  {data.name}
                </option>
          ...
          <SelectInput label="Gênero" {...register("genero")}>
            {gender.map((data) => (
              <option
                style={{ color: "black" }}
                value={data.id}
                selected={values.gender_id == data.id}
                key={data.id}
              >
                {data.name}
              </option>
          ...
          <Button type="submit" colorScheme="messenger" onClick={notify}>
            Editar
          </Button>
          <Button
            bg="gray.600"
